...
class SetupAppCredentialsTests(unittest.TestCase):
    def test_config_candidates_prefer_v2_then_legacy_namespace(self) -> None:
        with patch.dict(MODULE.os.environ, {}, clear=True):
            candidates = MODULE.config_candidates()

        self.assertEqual(
            candidates[0].parts[-3:],
            ("soia-skills", MODULE.SKILL_NAME, "config.yml"),
        )
        self.assertEqual(
            candidates[1].parts[-5:],
            (
                "soia-skills",
                MODULE.LEGACY_REPO,
                MODULE.LEGACY_SKILL_TYPE,
                MODULE.SKILL_NAME,
                "config.yml",
            ),
        )
