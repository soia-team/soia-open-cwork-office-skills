...
思维导图编辑器使用一级 `导出为`，随后精确选择计划中的 `Xmind文件`；ProcessOn 当前编辑器会把同一项渲染为 `XMind文件`，技能只在固定 provider-label 白名单内兼容这一大小写变体；不得先寻找流程图专用的“文件”菜单。菜单定位同时读取可见正文和已知的 `aria-label`、`title`、`data-title`、`data-tooltip` 属性，但属性选择器是技能内固定菜单名白名单，不接收行动文件或图表标题提供的 CSS。若一级导出控件在有界等待内仍不可见，回执记录 `editor_export_controls_unavailable` 和页面类型，保留为定向诊断而不是重复盲试。
...
默认 profile 位于用户私有 config 根的 `soia-skills/soia-cwork-processon-diagrams/browser-profile/`。可用 `--profile-dir` 或 `SOIA_CWORK_PROCESSON_BROWSER_PROFILE_DIR` 覆盖，但 runner 会拒绝系统默认 Chrome/Chromium profile、符号链接和非空无技能标记目录。profile 由浏览器保存登录态；技能不读取其 Cookie、Storage、密码或凭据文件。
...
当经过真实 proof 的批处理需要跨越宿主单次执行时限时，可由 `processon_archive_supervisor.py` 运行**有上限的顺序小批次**。它仍只使用技能专用 headless profile；不是 Codex/Claude computer-use，也不会操作客户主 Chrome。每批结束后强制重放 `audit`，并将运行历史原子写入 progress 同目录的私有 `archive-supervisor-state.json`。
...
python3 scripts/processon_archive_supervisor.py \
  ...
  --progress <run-dir>/artifacts/download-progress.json \
  --team-url '<team-url>' --config <private-config.yml> \
  --profile-dir <dedicated-profile> \
  --concurrency-proof <run-dir>/artifacts/concurrency-proof.json \
  --workers 2 --limit 8 --timeout-ms 15000 --max-batches 20
```
