# AGENTS.md - soia-open-cwork-office-skills
...
`soia-open-cwork-office-skills` publishes reusable `soia-cwork-*` skills for the cwork domain. Every committed skill must be safe for users who do not share the maintainer's machine, accounts, private data, or internal workspace.
## Routing Boundary
...
```bash
npx skills add soia-team/soia-open-cwork-office-skills -l --full-depth
npx skills add soia-team/soia-open-cwork-office-skills -g --all
```
...
```bash
claude plugin update soia-cwork-office@soia   # 交付走插件市场，勿装全局
```

Forbidden outside local testing:

```bash
npx skills add /absolute/local/path/to/soia-open-cwork-office-skills -g -a '*' -s <skill-name> -y
```
...
```bash
cd <your-local-checkout>/soia-open-cwork-office-skills
...
3. **Users update** with `claude plugin update soia-cwork-office@soia` or
...
claude plugin update soia-cwork-office@soia
```
